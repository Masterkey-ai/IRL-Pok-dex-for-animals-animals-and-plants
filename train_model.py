"""
NatureDex AI — Phase 4 Model Training
======================================
Fine-tunes EfficientNetV2-S on the downloaded NC wildlife dataset.
Run this on your Windows PC with the RTX 5070 Super.
...
Output:
    models/
    ├── naturedex_nc_v1.pth       (final trained model weights)
    ├── naturedex_nc_v1.onnx      (ONNX export for app integration)
    ├── label_map.json            (class index → species info)
...
BATCH_SIZE   = 32           # safe for 12GB+ VRAM; lower to 16 if OOM
NUM_EPOCHS   = 30           # phase 1: freeze backbone, train head
FINETUNE_EPOCHS = 15        # phase 2: unfreeze all, fine-tune end-to-end
LR_HEAD      = 1e-3         # learning rate for the classification head
LR_FINETUNE  = 3e-5         # much lower LR when fine-tuning the whole model
WEIGHT_DECAY = 1e-4
NUM_WORKERS  = 4            # data loading threads; reduce to 0 if Windows errors
...
# Training: aggressive augmentation to prevent overfitting on wildlife photos
...
def load_datasets():
    ...
    if not train_dir.exists():
        raise FileNotFoundError(
            ...
            "Make sure you copied the full dataset/ folder from your Mac."
        )
...
def build_model(num_classes: int):
    """
    Load EfficientNetV2-S pretrained on ImageNet, replace the classifier head
    with one sized for our NC species count.
    EfficientNetV2-S: fast, accurate, 20M params — great for fine-tuning.
    """
    model = models.efficientnet_v2_s(weights=models.EfficientNet_V2_S_Weights.DEFAULT)

    # Freeze all backbone layers — we only train the new head first
    ...
    # New head layers are unfrozen by default
...
def train_one_epoch(model, loader, criterion, optimizer, scaler):
    ...
    for inputs, labels in tqdm(loader, desc="  train", leave=False):
        ...
        # Mixed precision — cuts memory use and speeds up 5070 Super significantly
...
def save_label_map(classes: list[str], source_label_map: dict):
    """
    Save a final label map that maps class index (int) → species metadata.
    The app uses this to convert model output indices to species names.
    """
    label_map_out = {}
...
def export_onnx(model, num_classes: int):
    """Export the trained model to ONNX so it can run on any platform."""
    model.eval()
...
def main():
    print("=" * 60)
    print("NatureDex AI — Custom NC Model Training")
    ...
    # Load source label map from download script
    source_label_map = {}
    ...
    with open(log_path, "w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=["epoch","phase","train_loss","train_acc","val_loss","val_acc"])
        writer.writeheader()
    ...
    print(f"\nNext: copy models/ folder back to your Mac")
    print(f"and run integrate_model.py to add it to NatureDex")
    print(f"{'='*60}")
